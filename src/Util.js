...
async function displayFilteredTrashItems(currentPage, searchTerm, trashItems) {
  ...
  if (typeof currentPage !== "number" || currentPage < 1) {
    throw new TypeError("The 'currentPage' parameter must be a positive number.");
  }
  if (typeof searchTerm !== "string") {
    throw new TypeError("The 'searchTerm' parameter must be a string.");
  }
  if (!Array.isArray(trashItems)) {
    throw new TypeError("The 'trashItems' parameter must be an array of strings.");
  }
  ...
  for (let i = startIdx; i < endIdx; i++) {
    ...
    image.src = imgSrc;
    image.addEventListener('click', () => displayItemInfo(item, imgSrc, image, isTrashMode = true));
...
function handleDisplayChange(element, searchKeyword) {
  ...
  let [all_tag_id, ob46_tag_id, ob47_tag_id, trashItem_btn] = ["AllItem_btn", "Ob46Item_btn", "Ob47Item_btn", "trashItem_btn"].map(id => document.getElementById(id));
...
function addEnterKeyListener(inputElement, searchFunction) {
  if (!inputElement || typeof searchFunction !== 'function') {
    console.error("Invalid parameters: Input element or search function is missing.");
    return;
  }
...
function displayItemInfo(itemData, imageSource, sharedElement, isTrashMode) {
  ...
  if (!isTrashMode) {
    ...
    [dialogTitleParagraphs.hedear, dialogTitleParagraphs.title, dialogTitleParagraphs.iconName].forEach((element, index) => {
    ...
    [dialogTitleParagraphs.title, dialogTitleParagraphs.iconName].forEach(element => {
  ...
  dialogTitleParagraphs.closeBtn.addEventListener('click', () => {
    ...
    setTimeout(() => {
      pageBackgrounds.forEach(id => {
        document.getElementById(id).style.animation = "fadeOut 300ms 1 forwards";
      });
    }, 250);
 ...
 */
async function goToPage(pageNumber, searchTerm, webps, isTrashMode, totalPages) {
  // Validate input
